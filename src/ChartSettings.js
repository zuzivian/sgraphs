...
function ChartSettings(props) {
  ...
  useEffect(() => {
    ...
    setDatasetMenu(menu);
  }, [organisation, props.packages, resourceID, packages, filterResourceIDs]);
  ...
  return (
    <Form className="settingsbar px-3 py-3">
        ...
        <Row>
          ...
          <Col>
            ...
            >
              {datasetMenu.map((key) => (
                <option key={key} value={key}>
                  {getResourceNamefromID(props.packages, key)}
                </option>
              ))}
            </Form.Select>
          </Col>
      ...
      <Form.Group controlId="dataset">
        <Row>
          <h3 className="mb-3">Data Manipulation</h3>
        </Row>
        <Row>
          <Col xs={4}>
          ...
          <Col>
            <Form.Select value={xKey} onChange={(e) => setXKey(e.target.value)}>
              {fields.map((key) => (
                <option key={key} value={key}>
                  {key}
                </option>
        ...
        <Row>
          ...
          <Col>
            <Form.Select value={yKey} onChange={(e) => setYKey(e.target.value)}>
              {fields.map((key) => (
                <option key={key} value={key}>
                  {key}
                </option>
        ...
        <Row>
          <Col xs={6}>
            <Form.Label>Sum Y-axis data entries?</Form.Label>
          </Col>
          <Col>
            <Form.Check
              type="checkbox"
              checked={sumData}
              onChange={(e) => setSumData(!sumData)}
            />
          </Col>
        </Row>
      </Form.Group>
      <Form.Group controlId="xyAxisDomain">
        <Row>
          <h3 className="mb-3">Chart Range</h3>
        </Row>
        <Row>
          <Col>
            <Form.Text>Min X</Form.Text>
          </Col>
          <Col>
            <Form.Control
              type="text"
        ...
        <Row>
          <Form.Text>
            Valid inputs: a number, 'auto', 'dataMin', 'dataMax', or a string
            like 'dataMin - 20', 'dataMax + 100'
          </Form.Text>
        </Row>
      </Form.Group>
      <Row>
        <Button
          target="_blank"
          href={
            "https://data.gov.sg/search?q=" +
            getResourceNamefromID(props.packages, resourceID)
          }
        >
          View on data.gov.sg
        </Button>
