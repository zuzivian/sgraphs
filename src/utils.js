...
export function computeLabels(fields, records) {
  ...
  for (let item in fields) {
    ...
    if (
      field_name === "_id" ||
      field_name.includes("year") ||
      field_name.includes("month")
    )
      continue;
    if (
      (!temp_y && isFloatOrInt(sample_data)) ||
      field_name.includes("value") ||
      field_name.includes("amt") ||
      field_name.includes("amount")
    ) {
      temp_y = field_name;
    }
  }
  ...
  for (let item in fields) {
    ...
    if (
      (!temp_x && isFloatOrInt(sample_data)) ||
      field_name.includes("year") ||
      field_name.includes("month")
    ) {
      temp_x = field_name;
  ...
  for (let item in fields) {
    let field_name = fields[item].id;
    let new_unique = new Set(
      records.map((item, field_name) => item[field_name])
    );
    let old_unique = new Set(
      records.map((item, temp_series) => item[temp_series])
    );
    if (field_name === "_id" || field_name === temp_y || field_name === temp_x)
      continue;
    if (
      new_unique.size < 20 &&
      new_unique.size > 1 &&
      (!temp_series || new_unique.size < old_unique.size)
    ) {
